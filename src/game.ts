...
function paintDock(ctx: CanvasRenderingContext2D, t: number, state: GameState) {
  ...
  // pool deck (floor) — pale tiles with grout
  ctx.fillStyle = "#b9ac90";
  ...
  // the pool (deep end) — blue water with coping
  ctx.fillStyle = "#cdbfa6";
  ctx.fillRect(148, 84, 48, 4); // coping
  ctx.fillStyle = "#2f7fb0";
  ctx.fillRect(150, 88, 44, 22);
  ctx.fillStyle = "#1d5f8a";
  ctx.fillRect(156, 94, 32, 14); // deeper water
  ctx.fillStyle = "rgba(255,255,255,0.18)";
  for (let i = 0; i < 4; i++) ctx.fillRect(154 + ((t * 6 + i * 12) % 36), 90 + i * 4, 6, 1);
  if (!state.flags.gotKey) {
    ctx.fillStyle = "rgba(220,210,150,0.7)";
    ctx.fillRect(170, 101, 3, 2); // glint
  }

  // sign post
  ctx.fillStyle = "#8a7250";
  ctx.fillRect(104, 90, 4, 16);
  ctx.fillStyle = "#d24a6a"; // hot-pink reality-show sign
  ctx.fillRect(96, 78, 22, 14);
  ctx.fillStyle = "#fff";
  ctx.fillRect(99, 82, 16, 1);
  ctx.fillRect(99, 85, 12, 1);

  // sun lounger + pool skimmer (right) — hide skimmer once taken
  ctx.fillStyle = "#e8e2d4";
  ctx.fillRect(244, 100, 30, 8);
  ctx.fillStyle = "#cfc7b4";
  ctx.fillRect(244, 100, 30, 2);
  ...
  // the pelican, watching from the deck
